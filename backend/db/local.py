...
class StoryDatabase():
    ...
    def next_story_name(previous_story_name):
        return f"{int(previous_story_name[:-3]) + 1}.md"
    ...
    def save_new_story(self, story: str):
        next_story_name = self.next_story_name(self.latest_story_name())
        with open(next_story_name, "w") as f:
            f.write(story)
